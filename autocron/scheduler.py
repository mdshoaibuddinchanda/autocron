...
class AutoCron:
        ...
        for channel in channels:
            if channel == 'desktop':
                ...
                if task.email_config:
                    ...
                    self.logger.warning(f"Email notification requested but no config provided for task '{task.name}'")
    ...
    def _register_os_task(self, task: Task) -> None:
        ...
        try:
            cron_expr = task.schedule_value if task.schedule_type == 'cron' else self._interval_to_cron(task.schedule_value)
    ...
    def from_config(cls, config_path: str) -> 'AutoCron':
        ...
        for task_config in config.get('tasks', []):
            scheduler.add_task(
                ...
                every=task_config.get('schedule') if '/' not in task_config.get('schedule', '') else None,
                cron=task_config.get('schedule') if '/' in task_config.get('schedule', '') else None,
