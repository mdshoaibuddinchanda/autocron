...
class Dashboard:
    ...
    def show_live_monitor(self, refresh_rate: int = 2) -> None:
        ...
        try:
            with Live(self._generate_live_view(), refresh_per_second=1/refresh_rate, console=self.console) as live:
                while True:
                    import time
    ...
    def _generate_live_view(self) -> Table:
        ...
        if not stats:
            ...
            for task_stat in stats:
                status = "✅" if task_stat["success_rate"] >= 95 else "⚠️" if task_stat["success_rate"] >= 80 else "❌"
                last_run = datetime.fromisoformat(task_stat["last_run"])
                time_ago = self._format_time_ago(last_run)
