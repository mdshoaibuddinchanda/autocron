    ...
    def __init__(self):
        ...
        if self.plyer is None:
            raise NotificationError(
                ...
                "Install with: pip install autocron-scheduler[notifications]"
            )
...
class NotificationManager:
    ...
    def add_notifier(self, name: str, notifier: Notifier) -> None:
        ...
        """
        if not name:
            raise ValueError("Notifier name cannot be empty")
        if not callable(getattr(notifier, "send", None)):
            raise TypeError("notifier must provide a callable send method")
        with self._lock:
            self.notifiers[name] = notifier

    def remove_notifier(self, name: str) -> bool:
        """Remove a notifier without affecting other task-scoped channels."""
        with self._lock:
            return self.notifiers.pop(name, None) is not None

    def setup_desktop(self, name: str = "desktop") -> Optional[str]:
        """Set up desktop notifications and return the registered channel."""
        with contextlib.suppress(NotificationError):
            notifier = DesktopNotifier()
            self.add_notifier(name, notifier)
            return name
        return None

    def setup_email(self, config: Dict[str, Any], name: str = "email") -> str:
        """
        Set up email notifications.

        Args:
            config: Email configuration dictionary
            name: Channel name. Task schedulers use a task-scoped value so one
                task cannot overwrite another task's SMTP configuration.
    ...
    def notify(
        self, title: str, message: str, channels: Optional[list] = None, **kwargs: Any
    ) -> Dict[str, bool]:
        """
        ...
        """
        with self._lock:
            selected_channels = list(self.notifiers) if channels is None else list(channels)
            selected_notifiers = {
                channel: self.notifiers.get(channel) for channel in selected_channels
            }

        results = {}
        for channel, notifier in selected_notifiers.items():
