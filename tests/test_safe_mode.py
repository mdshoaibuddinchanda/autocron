...
def test_safe_mode_basic_execution(scheduler, safe_script):
    ...
    task_id = scheduler.add_task(
        name="safe_task", script=safe_script, every="1h", safe_mode=True, timeout=5
    )
...
def test_safe_mode_isolation(scheduler, safe_script):
    ...
    task_id = scheduler.add_task(
        name="isolated_task", script=safe_script, every="1h", safe_mode=True, timeout=5
    )
...
def test_safe_mode_default_disabled(scheduler, safe_script):
    ...
    task_id = scheduler.add_task(name="default_task", script=safe_script, every="1h")
...
def test_safe_mode_with_retries(scheduler, safe_script):
    ...
    task_id = scheduler.add_task(
        name="retry_safe_task", script=safe_script, every="1h", safe_mode=True, retries=3, timeout=5
    )
...
def test_safe_mode_function_tasks_unsupported(scheduler):
    ...
    task_id = scheduler.add_task(
        name="func_task", func=my_func, every="1h", safe_mode=True  # Should be set but not used
    )
